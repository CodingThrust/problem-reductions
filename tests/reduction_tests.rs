...
/// Tests for ReductionGraph path finding.
mod reduction_graph_tests {
    use super::*;
    ...
    fn test_no_path_exists() {
        ...
        // No reduction from QUBO<f64> to SetPacking<i32> (different type families)
        let paths = graph.find_paths::<QUBO<f64>, SetPacking<i32>>();
        assert!(paths.is_empty());
    }
