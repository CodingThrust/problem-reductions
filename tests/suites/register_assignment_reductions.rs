...
fn test_ksat_to_fra_structure_and_closed_loop_via_ilp() {
    ...
    let fra_solution = fra_chain
        .recover_result::<FeasibleRegisterAssignment, ILP<i64>>(
            fra,
    ...
    let sat_solution = ksat_chain
        .recover_result::<KSatisfiability<K3>, FeasibleRegisterAssignment>(
            &source,
            SolveOutcome::optimal(fra, fra_solution).unwrap(),
        )
        .map(|outcome| outcome.into_solution().unwrap())
        .unwrap();
