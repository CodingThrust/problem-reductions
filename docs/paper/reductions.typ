...
Shown NP-complete by Karp (1972) via transformation from 3-Dimensional Matching @karp1972. X3C remains NP-complete even when no element appears in more than three subsets, but is solvable in polynomial time when no element appears in more than two subsets. It is one of the most widely used source problems for NP-completeness reductions in Garey & Johnson (A3 SP2), serving as the starting point for proving hardness of problems in scheduling, graph theory, set systems, coding, and number theory. The best known exact algorithm is $O^*(2^m)$ by brute-force enumeration over the $m$ subsets.

*Example.* Let $X = {1, 2, dots, 9}$ ($q = 3$) and $cal(C) = {S_1, S_2, S_3, S_4, S_5}$ with $S_1 = {1, 2, 3}$, $S_2 = {4, 5, 6}$, $S_3 = {7, 8, 9}$, $S_4 = {1, 4, 7}$, $S_5 = {2, 5, 8}$. An exact cover is $cal(C)' = {S_1, S_2, S_3}$: each element of $X$ appears in exactly one triple, and $|cal(C)'| = 3 = q$. No other subcollection of size 3 forms an exact cover.
]
