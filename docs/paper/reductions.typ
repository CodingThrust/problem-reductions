...
#problem-def("MultiprocessorScheduling")[
  Given a multiset of task lengths $ell_1, ell_2, dots, ell_n in ZZ^+_0$, a positive integer $m$, and a deadline $D in ZZ^+$, determine whether the tasks can be assigned to $m$ identical processors so that the total assigned load on every processor is at most $D$.
][
  Multiprocessor Scheduling is the classical decision problem SS8 in Garey and Johnson @garey1979. It generalizes Partition: when $m = 2$ and $D$ is half the total work, feasibility asks whether the jobs can be split into two equal-load groups. For any fixed number of processors, dynamic programming gives pseudo-polynomial algorithms, but when $m$ is part of the input the problem is strongly NP-complete; see the scheduling complexity survey of Brucker, Lenstra, and Rinnooy Kan @brucker1977. The brute-force formulation used by this project checks all $m^n$ task-to-processor assignments; subset-based exact partitioning methods improve the general bound to $O^*(2^n)$ @bjorklund2009.
