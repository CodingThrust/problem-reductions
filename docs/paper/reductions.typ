...
#reduction-rule("MultipleCopyFileAllocation", "ILP")[
  ...
  _Construction._ Variables: binary $x_v$ (copy at $v$) and $y_(v,u)$ (vertex $v$ served by copy at $u$). Constraints: $sum_u y_(v,u) = 1$ (assignment); $y_(v,u) <= x_u$ (capacity link). Objective: minimize $sum_v s_v x_v + sum_(v,u) "usage"_v dot d(v, u) dot y_(v,u)$.
...
#reduction-rule("ExpectedRetrievalCost", "ILP")[
  ...
  _Construction._ Variables: binary $x_(r,s)$ (record $r$ in sector $s$), one-hot per record, plus linearization variables $z_((r,s),(r',s')) = x_(r,s) dot x_(r',s')$. Constraints: one-hot assignment; McCormick linearization ($z <= x$, $z <= y$, $z >= x + y - 1$). Objective: minimize $sum d(s,s') p_r p_(r') z_((r,s),(r',s'))$.
...
#reduction-rule("SumOfSquaresPartition", "ILP")[
  ...
  _Construction._ Variables: binary $x_(i,g)$ (element $i$ in group $g$), plus $z_((i,j),g) = x_(i,g) dot x_(j,g)$. Constraints: one-hot assignment; McCormick linearization for $z$. Objective: minimize $sum_g sum_(i,j) s_i s_j z_((i,j),g)$.
