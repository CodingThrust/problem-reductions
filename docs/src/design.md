...
`ReductionGraph::find_executable_path::<S, T>()` returns a typed `ExecutablePath<S, T>` that chains reductions automatically via type erasure. Call `reduce()` to execute the path:
...
Internally, `ExecutablePath` holds a `Vec<fn(&dyn Any) -> Box<dyn DynReductionResult>>` — one type-erased executor per edge. Each `reduce_fn` is generated by the `#[reduction]` macro and downcasts the source, calls `ReduceTo::reduce_to()`, and boxes the result. `ChainedReduction` stores the intermediate results and extracts solutions back through the chain in reverse.
