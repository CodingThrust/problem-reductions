...
fn accept_backend_status(status: HighsModelStatus) -> Result<(), ILPSolveError> {
    match status {
        HighsModelStatus::Optimal => Ok(()),
        HighsModelStatus::Infeasible => Err(ILPSolveError::Infeasible),
        HighsModelStatus::Unbounded => Err(ILPSolveError::Unbounded),
        HighsModelStatus::ReachedTimeLimit => Err(ILPSolveError::Timeout),
        other => Err(ILPSolveError::BackendFailure(format!(
            "HiGHS status: {other:?}"
        ))),
    }
}
...
impl HighsAdapter {
        ...
        C: BackendCoefficient,
    {
        ...
        let columns = problem
            ...
            .collect::<Result<Vec<_>, ILPSolveError>>()?;
        ...
        let mut model = backend.try_optimise(sense).map_err(|error| {
            ILPSolveError::BackendFailure(format!("loading HiGHS model: {error:?}"))
        })?;
        model.make_quiet();
        for (option, value) in [("random_seed", 0), ("threads", 1)] {
            model.try_set_option(option, value).map_err(|error| {
                ILPSolveError::BackendFailure(format!("setting {option}: {error:?}"))
            })?;
        }
        ...
        model.try_set_option("parallel", "off").map_err(|error| {
            ILPSolveError::BackendFailure(format!("setting parallel: {error:?}"))
        })?;
        if let Some(seconds) = self.time_limit {
            model
                .try_set_option("time_limit", seconds)
                .map_err(|error| {
                    ILPSolveError::BackendFailure(format!("setting time_limit: {error:?}"))
                })?;
        }
        let solved = model
            .try_solve()
            .map_err(|error| ILPSolveError::BackendFailure(format!("running HiGHS: {error:?}")))?;
