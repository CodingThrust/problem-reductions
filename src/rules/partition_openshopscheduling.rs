...
impl ReductionResult for ReductionPartitionToOpenShopScheduling {
    ...
    fn extract_solution(
        ...
        Ok({
            let num_elements = self.target.inner().num_jobs() - 1;
            let mut source_config = vec![false; num_elements];
            let m = self.target.inner().num_machines();
            ...
            let half_sum = self.target.inner().processing_times()[special_job][0];
            ...
            for (job, slot) in source_config.iter_mut().enumerate() {
                let completion = start_times[job][middle_machine]
                    .checked_add(self.target.inner().processing_times()[job][middle_machine])
                    .ok_or_else(|| {
