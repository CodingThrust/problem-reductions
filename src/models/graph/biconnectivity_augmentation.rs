    ...
    W::Sum: Deserialize<'de>,
{
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Self::new(data.graph, data.potential_weights, data.budget).map_err(serde::de::Error::custom)
    }
}
    ...
    for BiconnectivityAugmentation<SimpleGraph, i64>
{
    ...
    fn try_from(spec: BiconnectivityAugmentationCreateSpec) -> Result<Self, Self::Error> {
        ...
        Self::new(graph, spec.potential_weights, spec.budget)
    }
}
...
impl<G: Graph, W: WeightElement> BiconnectivityAugmentation<G, W> {
    ...
    ///
    /// # Errors
    /// Returns an error if any potential edge references a vertex index outside the graph,
