    ...
    W::Sum: Deserialize<'de>,
{
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Self::new(data.graph, data.weights, data.k, data.bound).map_err(serde::de::Error::custom)
    }
}
...
impl TryFrom<KthBestSpanningTreeCreateSpec> for KthBestSpanningTree<i64> {
    ...
    fn try_from(spec: KthBestSpanningTreeCreateSpec) -> Result<Self, Self::Error> {
        ...
        Self::new(graph, weights, spec.k, spec.bound)
    }
}
...
impl<W: WeightElement> KthBestSpanningTree<W> {
    ...
    /// # Errors
    ///
    /// Returns an error if the number of weights does not match the number of edges, or
    /// if `k` is zero.
    pub fn new(
        graph: SimpleGraph,
        weights: Vec<W>,
...
pub(crate) fn canonical_model_example_specs() -> Vec<crate::example_db::specs::ModelExampleSpec> {
    ...
    let problem = KthBestSpanningTree::new(graph, vec![1, 1, 2, 2, 2, 3], 2, 4).unwrap();
