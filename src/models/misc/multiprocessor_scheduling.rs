...
//!
//! Given a set of tasks with processing times, a number of identical processors,
//! and a global deadline, determine whether the tasks can be partitioned among
//! the processors so that every processor's total load is at most the deadline.
...
inventory::submit! {
    ProblemSchemaEntry {
        ...
        description: "Assign tasks to identical processors so that no processor load exceeds a deadline",
        fields: &[
...
///
/// The original scheduling formulation assigns start times to tasks, but for
/// identical non-preemptive processors with no precedence constraints, a
/// schedule is feasible if and only if the tasks can be partitioned among the
/// processors so that each processor's total load is at most the deadline.
///
/// # Representation
///
...
crate::declare_variants! {
    default sat MultiprocessorScheduling => "num_processors ^ num_tasks",
}
