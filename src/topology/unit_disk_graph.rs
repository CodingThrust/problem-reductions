...
impl UnitDiskGraph {
    ...
    pub fn bounding_box(&self) -> Option<((f64, f64), (f64, f64))> {
        ...
        let min_x = self.positions.iter().map(|p| p.0).fold(f64::INFINITY, f64::min);
        let max_x = self.positions.iter().map(|p| p.0).fold(f64::NEG_INFINITY, f64::max);
        let min_y = self.positions.iter().map(|p| p.1).fold(f64::INFINITY, f64::min);
        let max_y = self.positions.iter().map(|p| p.1).fold(f64::NEG_INFINITY, f64::max);
