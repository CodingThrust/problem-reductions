...
fn test_set_splitting_evaluate_multiple_subsets() {
    ...
    let config = vec![true, false, true, false, false, true];
    assert_eq!(problem.evaluate(&config).unwrap(), Or(true));

    // All 0: every subset is monochromatic
    let all_zero = vec![false, false, false, false, false, false];
...
fn test_set_splitting_is_valid_solution() {
    ...
    assert!(problem
        .is_valid_solution(&[false, true, false, true])
        .unwrap());
    assert!(!problem
        .is_valid_solution(&[false, false, false, false])
        .unwrap());
}
