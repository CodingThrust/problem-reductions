...
fn test_three_dimensional_matching_evaluation() {
    ...
    assert!(problem
        .evaluate(&vec![true, true, true, false, false])
        .unwrap());
    ...
    assert!(!problem
        .evaluate(&vec![true, true, false, true, false])
        .unwrap());
...
fn test_three_dimensional_matching_solver() {
    ...
    assert!(solutions.contains(&vec![true, true, true, false, false]));
}

#[test]
fn test_three_dimensional_matching_no_solution() {
    // q = 2, all triples share w=0 -> no matching of size 2 possible
    let problem = ThreeDimensionalMatching::new(2, vec![(0, 0, 0), (0, 1, 1), (0, 0, 1)]).unwrap();
...
fn test_three_dimensional_matching_serialization() {
    let problem = ThreeDimensionalMatching::new(2, vec![(0, 1, 0), (1, 0, 1)]).unwrap();
...
fn test_three_dimensional_matching_empty() {
    ...
    let problem = ThreeDimensionalMatching::new(0, vec![]).unwrap();
    assert!(problem.evaluate(&vec![]).unwrap());
    let solver = BruteForce::new();
    let solutions = solver.find_all_witnesses(&problem).unwrap();
    assert_eq!(solutions, vec![Vec::<bool>::new()]);
}
...
fn test_three_dimensional_matching_rejects_non_binary_config_values() {
    let problem = ThreeDimensionalMatching::new(2, vec![(0, 1, 0), (1, 0, 1)]).unwrap();
    assert!(
        crate::registry::DynProblem::evaluate_dyn(&problem, &serde_json::json!([true, 2])).is_err()
    );
}

#[test]
fn test_three_dimensional_matching_element_out_of_range() {
    assert!(ThreeDimensionalMatching::new(2, vec![(0, 3, 0)]).is_err());
}

#[test]
fn test_three_dimensional_matching_is_valid_solution() {
    let problem = ThreeDimensionalMatching::new(2, vec![(0, 1, 0), (1, 0, 1)]).unwrap();
    assert!(problem.evaluate(&vec![true, true]).unwrap().0);
    assert!(!problem.evaluate(&vec![true, false]).unwrap().0);
}
...
fn test_three_dimensional_matching_duplicate_coordinates() {
    ...
    let problem = ThreeDimensionalMatching::new(2, vec![(0, 0, 0), (1, 1, 1), (0, 1, 0)]).unwrap();
