...
fn test_subsetproduct_evaluate_satisfying() {
    let problem = SubsetProduct::new(vec![2u32, 3, 5, 7, 6, 10], 210u32);
    // {2, 3, 5, 7} = 210
    assert!(problem.evaluate(&[1, 1, 1, 1, 0, 0]).unwrap());
    // {3, 7, 10} = 210
    assert!(problem.evaluate(&[0, 1, 0, 1, 0, 1]).unwrap());
}
...
fn test_subsetproduct_all_selected() {
    ...
    let problem = SubsetProduct::new(vec![2u32, 3, 5], 30u32);
    assert!(problem.evaluate(&[1, 1, 1]).unwrap()); // 2*3*5 = 30
}
...
fn test_subsetproduct_target_one() {
    ...
    assert!(problem.evaluate(&[0, 0, 0]).unwrap()); // empty subset product = 1
    assert!(!problem.evaluate(&[1, 0, 0]).unwrap()); // 2 != 1
}
...
fn test_subsetproduct_large_integer_input() {
    ...
    assert!(problem.evaluate(&[1, 1, 1, 1, 0, 0]).unwrap()); // 2*3*5*7 = 210
    assert!(!problem.evaluate(&[1, 1, 0, 0, 0, 0]).unwrap()); // 2*3 = 6
}
