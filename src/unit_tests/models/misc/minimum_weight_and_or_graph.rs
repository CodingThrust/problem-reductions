...
fn test_minimum_weight_and_or_graph_evaluate_optimal() {
    ...
    // Weights: 1+2+1+2 = 6
    assert_eq!(problem.evaluate(&[1, 1, 0, 1, 0, 1]).unwrap(), Min(Some(6)));
}
...
fn test_minimum_weight_and_or_graph_evaluate_all_arcs() {
    ...
    assert_eq!(
        problem.evaluate(&[1, 1, 1, 1, 1, 1]).unwrap(),
        Min(Some(13))
    );
}
...
fn test_minimum_weight_and_or_graph_and_violated() {
    ...
    assert_eq!(problem.evaluate(&[1, 0, 0, 1, 0, 1]).unwrap(), Min(None));
}
...
fn test_minimum_weight_and_or_graph_or_violated() {
    ...
    assert_eq!(problem.evaluate(&[1, 1, 0, 0, 0, 1]).unwrap(), Min(None));
}
...
fn test_minimum_weight_and_or_graph_dangling_arc() {
    ...
    assert_eq!(problem.evaluate(&[0, 0, 1, 0, 0, 0]).unwrap(), Min(None));
}
...
fn test_minimum_weight_and_or_graph_solver() {
    ...
    assert_eq!(problem.evaluate(&w).unwrap(), Min(Some(6)));
}
...
fn test_minimum_weight_and_or_graph_serialization() {
    ...
    assert_eq!(
        round_trip.evaluate(&[1, 1, 0, 1, 0, 1]).unwrap(),
        Min(Some(6))
    );
}
...
fn test_minimum_weight_and_or_graph_paper_example() {
    ...
    let optimal_witnesses: Vec<_> = all
        .into_iter()
        .filter(|w| problem.evaluate(w).unwrap() == Min(Some(6)))
        .collect();
