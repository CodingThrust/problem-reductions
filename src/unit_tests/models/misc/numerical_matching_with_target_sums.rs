...
fn test_nmts_evaluate_valid() {
    ...
    assert_eq!(problem.evaluate(&vec![0, 2, 1]).unwrap(), Or(true));
}
...
fn test_nmts_evaluate_invalid_sums() {
    ...
    assert_eq!(problem.evaluate(&vec![0, 1, 2]).unwrap(), Or(false));
    // config [1,0,2] → sums: 1+5=6, 4+2=6, 7+3=10 → multiset {6,6,10} ≠ {3,7,12}
    assert_eq!(problem.evaluate(&vec![1, 0, 2]).unwrap(), Or(false));
}
