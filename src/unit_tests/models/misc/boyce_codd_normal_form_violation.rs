...
fn test_bcnf_evaluate_invalid_config_length() {
    let problem = canonical_problem();
    assert!(matches!(
        problem.evaluate(&vec![false, false, true, false, false]),
        Err(crate::traits::EvaluationError::InvalidConfiguration(_))
    ));
    assert!(matches!(
        problem.evaluate(&vec![false, false, true, false, false, false, false]),
        Err(crate::traits::EvaluationError::InvalidConfiguration(_))
    ));
}
...
fn test_bcnf_evaluate_invalid_config_values() {
    ...
    assert!(crate::registry::DynProblem::evaluate_dyn(
        &problem,
        &serde_json::json!([false, false, 2, false, false, false])
    )
    .is_err());
}
...
fn test_bcnf_solver_finds_violation() {
    ...
    assert!(solutions.contains(&vec![false, false, true, false, false, false]));
}
