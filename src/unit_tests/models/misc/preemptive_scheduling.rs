...
fn small_instance() -> PreemptiveScheduling {
    PreemptiveScheduling::new(vec![2, 1], 2, vec![]).unwrap()
}
...
fn test_preemptive_scheduling_creation() {
    let p = PreemptiveScheduling::new(vec![2, 1, 3], 2, vec![(0, 2)]).unwrap();
...
fn test_preemptive_scheduling_paper_example() {
    ...
    // Optimal makespan = 5
    let p = PreemptiveScheduling::new(vec![2, 1, 3, 2, 1], 2, vec![(0, 2), (1, 3)]).unwrap();
    let d = p.d_max(); // = 9
    assert_eq!(d, 9);
...
fn test_preemptive_scheduling_serialization_roundtrip_evaluate() {
    let p = PreemptiveScheduling::new(vec![1, 1], 2, vec![(0, 1)]).unwrap();
    let json = serde_json::to_value(&p).unwrap();
...
fn test_preemptive_scheduling_zero_length() {
    assert!(PreemptiveScheduling::new(vec![0, 1], 2, vec![]).is_err());
}

#[test]
fn test_preemptive_scheduling_zero_processors() {
    assert!(PreemptiveScheduling::new(vec![1, 1], 0, vec![]).is_err());
}

#[test]
fn test_preemptive_scheduling_precedence_out_of_range() {
    assert!(PreemptiveScheduling::new(vec![1, 1], 2, vec![(0, 5)]).is_err());
}
