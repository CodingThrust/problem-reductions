...
fn test_maximum_likelihood_ranking_evaluate_optimal() {
    ...
    // (3,2): matrix[3][2] = 1
    // Total = 1 + 2 + 1 + 0 + 2 + 1 = 7
    assert_eq!(problem.evaluate(&vec![0, 1, 2, 3]).unwrap(), Min(Some(7)));
}
...
fn test_maximum_likelihood_ranking_evaluate_suboptimal() {
    ...
    assert_eq!(problem.evaluate(&vec![3, 2, 1, 0]).unwrap(), Min(Some(23)));
}
...
fn test_maximum_likelihood_ranking_two_items() {
    ...
    assert_eq!(problem.evaluate(&vec![1, 0]).unwrap(), Min(Some(3)));
...
fn test_maximum_likelihood_ranking_skew_symmetric() {
    ...
    assert_eq!(problem.evaluate(&vec![0, 1, 2]).unwrap(), Min(Some(-1)));
