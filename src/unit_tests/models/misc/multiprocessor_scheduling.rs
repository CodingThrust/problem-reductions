...
fn test_multiprocessor_scheduling_wrong_config_length() {
    let problem = MultiprocessorScheduling::new(vec![4, 5, 3], 2, 10).unwrap();
    assert!(matches!(
...
fn test_multiprocessor_scheduling_empty_instance() {
    let problem = MultiprocessorScheduling::new(vec![], 2, 10).unwrap();
...
fn test_multiprocessor_scheduling_single_task() {
    let problem = MultiprocessorScheduling::new(vec![5], 2, 5).unwrap();
...
fn test_multiprocessor_scheduling_single_task_exceeds_deadline() {
    let problem = MultiprocessorScheduling::new(vec![11], 2, 10).unwrap();
    assert!(!problem.evaluate(&vec![0]).unwrap());
    assert!(!problem.evaluate(&vec![1]).unwrap());
}
...
fn test_multiprocessor_scheduling_three_processors() {
    let problem = MultiprocessorScheduling::new(vec![3, 3, 3], 3, 3).unwrap();
    assert_eq!(
        crate::solvers::cartesian_dimensions(&problem).unwrap(),
        vec![3; 3]
    );
...
fn test_multiprocessor_scheduling_brute_force_infeasible() {
    ...
    let problem = MultiprocessorScheduling::new(vec![4, 5, 3, 2, 6], 2, 9).unwrap();
    let solver = BruteForce::new();
    let solution = solver.solve(&problem).unwrap();
...
fn test_multiprocessor_scheduling_zero_processors() {
    assert!(MultiprocessorScheduling::new(vec![1, 2], 0, 5).is_err());
}

#[test]
fn test_multiprocessor_scheduling_deadline_zero() {
    // Only feasible if all lengths are 0
    let problem = MultiprocessorScheduling::new(vec![0, 0], 2, 0).unwrap();
    assert!(problem.evaluate(&vec![0, 1]).unwrap());

    let problem2 = MultiprocessorScheduling::new(vec![1, 0], 2, 0).unwrap();
