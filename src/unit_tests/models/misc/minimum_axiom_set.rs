...
fn test_minimum_axiom_set_evaluate_insufficient() {
    ...
    let result = problem
        .evaluate(&vec![true, false, false, false, false, false, false, false])
        .unwrap();
    assert!(!result.is_valid());
}
...
fn test_minimum_axiom_set_evaluate_all_selected() {
    ...
    let result = problem
        .evaluate(&vec![true, true, true, true, true, true, true, true])
        .unwrap();
    assert!(result.is_valid());
...
fn test_minimum_axiom_set_evaluate_out_of_range() {
    ...
    assert!(crate::registry::DynProblem::evaluate_dyn(
        &problem,
        &serde_json::json!([2, false, false, false, false, false, false, false])
    )
    .is_err());
}
...
fn test_minimum_axiom_set_partial_true_sentences() {
    ...
    let result = problem.evaluate(&vec![false, false, true]).unwrap();
    assert!(!result.is_valid());
}
