...
fn test_non_tautology_is_falsifying() {
    let problem = NonTautology::new(3, vec![vec![1, 2], vec![-1, 3], vec![2, -3]]).unwrap();
    // x1=F, x2=F, x3=F:
...
fn test_non_tautology_variable_out_of_range() {
    assert!(NonTautology::new(2, vec![vec![1, 3]]).is_err());
}

#[test]
fn test_non_tautology_rejects_unrepresentable_literals_and_invalid_serde() {
    assert!(NonTautology::new(1, vec![vec![0]]).is_err());
    assert!(NonTautology::new(1, vec![vec![i64::MIN]]).is_err());
    let json = r#"{"num_vars":1,"disjuncts":[[0]]}"#;
    assert!(serde_json::from_str::<NonTautology>(json).is_err());
}
