...
fn test_json_enforces_construction_constraints() {
    let valid =
        serde_json::json!({"graph":{"num_vertices":3,"arcs":[[0,1],[1,2]]},"weights":[1,1]});
    let problem: MinimumFeedbackArcSet<i64> = serde_json::from_value(valid.clone()).unwrap();
    let encoded = serde_json::to_value(&problem).unwrap();
    let restored: MinimumFeedbackArcSet<i64> = serde_json::from_value(encoded.clone()).unwrap();
    assert_eq!(serde_json::to_value(&restored).unwrap(), encoded);
    let mut data = valid.clone();
    data["weights"] = serde_json::json!([]);
    assert!(
        serde_json::from_value::<MinimumFeedbackArcSet<i64>>(data.clone()).is_err(),
        "accepted {data}"
    );
}
...
fn test_minimum_feedback_arc_set_problem_name() {
    assert_eq!(
        <MinimumFeedbackArcSet<i64> as Problem>::NAME,
