...
fn test_json_enforces_construction_constraints() {
    ...
    let problem: MaxCut<SimpleGraph, i64> = serde_json::from_value(valid.clone()).unwrap();
    let encoded = serde_json::to_value(&problem).unwrap();
    let restored: MaxCut<SimpleGraph, i64> = serde_json::from_value(encoded.clone()).unwrap();
    assert_eq!(serde_json::to_value(&restored).unwrap(), encoded);
    let mut data = valid.clone();
    data["edge_weights"] = serde_json::json!([]);
    assert!(
        serde_json::from_value::<MaxCut<SimpleGraph, i64>>(data.clone()).is_err(),
        "accepted {data}"
    );
}
