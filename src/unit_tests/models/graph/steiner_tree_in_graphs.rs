...
fn create_spec_defaults_edge_weights() {
    let p = SteinerTreeInGraphs::try_from(SteinerTreeInGraphsCreateSpec::<i64> {
...
fn test_steiner_tree_creation() {
    ...
    let problem = SteinerTreeInGraphs::new(graph, vec![0, 3], vec![1i64, 2, 3]);
...
fn test_steiner_tree_is_valid_solution() {
    ...
    let problem = SteinerTreeInGraphs::new(graph, vec![0, 2], vec![1i64; 2]);
...
fn test_steiner_tree_problem_name() {
    assert_eq!(
        <SteinerTreeInGraphs<SimpleGraph, i64> as Problem>::NAME,
        "SteinerTreeInGraphs"
    );
}
...
fn test_steiner_tree_serialization() {
    ...
    let deserialized: SteinerTreeInGraphs<SimpleGraph, i64> = serde_json::from_str(&json).unwrap();
...
fn test_steiner_tree_edges_accessor() {
    let graph = SimpleGraph::new(3, vec![(0, 1), (1, 2)]);
    let problem = SteinerTreeInGraphs::new(graph, vec![0, 2], vec![5i64, 10]);
...
fn test_steiner_tree_weights_management() {
    let graph = SimpleGraph::new(3, vec![(0, 1), (1, 2)]);
    let mut problem = SteinerTreeInGraphs::new(graph, vec![0, 2], vec![1i64; 2]);
