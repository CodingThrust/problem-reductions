...
fn test_json_enforces_construction_constraints() {
    let valid =
        serde_json::json!({"graph":{"num_vertices":3,"edges":[[0,1],[1,2]]},"num_matchings":1});
    let problem: PartitionIntoPerfectMatchings<SimpleGraph> =
        serde_json::from_value(valid.clone()).unwrap();
    let encoded = serde_json::to_value(&problem).unwrap();
    let restored: PartitionIntoPerfectMatchings<SimpleGraph> =
        serde_json::from_value(encoded.clone()).unwrap();
    assert_eq!(serde_json::to_value(&restored).unwrap(), encoded);
    for (field, value) in [
        ("num_matchings", serde_json::json!(0)),
        ("num_matchings", serde_json::json!(4)),
    ] {
        let mut data = valid.clone();
        data[field] = value;
        assert!(
            serde_json::from_value::<PartitionIntoPerfectMatchings<SimpleGraph>>(data.clone())
                .is_err(),
            "accepted {data}"
        );
    }
...
fn test_partition_into_perfect_matchings_evaluate_positive() {
    ...
    assert!(problem.evaluate(&vec![0, 1, 0, 1]).unwrap());
}
...
fn test_partition_into_perfect_matchings_evaluate_negative() {
    ...
    assert!(!problem.evaluate(&vec![0, 0, 0, 1]).unwrap());
...
fn test_partition_into_perfect_matchings_evaluate_wrong_config_length() {
    ...
    assert!(matches!(
        problem.evaluate(&vec![0, 1, 0, 0, 1]),
        Err(crate::traits::EvaluationError::InvalidConfiguration(_))
    ));
}
