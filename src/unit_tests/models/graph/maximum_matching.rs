...
fn test_json_enforces_construction_constraints() {
    let valid =
        serde_json::json!({"graph":{"num_vertices":3,"edges":[[0,1],[1,2]]},"edge_weights":[1,1]});
    let problem: MaximumMatching<SimpleGraph, i64> = serde_json::from_value(valid.clone()).unwrap();
    let encoded = serde_json::to_value(&problem).unwrap();
    let restored: MaximumMatching<SimpleGraph, i64> =
        serde_json::from_value(encoded.clone()).unwrap();
    assert_eq!(serde_json::to_value(&restored).unwrap(), encoded);
    let mut data = valid.clone();
    data["edge_weights"] = serde_json::json!([]);
    assert!(
        serde_json::from_value::<MaximumMatching<SimpleGraph, i64>>(data.clone()).is_err(),
        "accepted {data}"
    );
}
...
fn test_is_valid_matching() {
    ...
    assert!(problem.is_valid_matching(&[true, false, true]));

    // Invalid: edges 0 and 1 share vertex 1
    assert!(!problem.is_valid_matching(&[true, true, false]));
}
...
fn test_matching_paper_example() {
    ...
    let config = vec![true, false, false, false, true, false];
    let result = problem.evaluate(&config).unwrap();
