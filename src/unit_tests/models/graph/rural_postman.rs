...
fn hexagon_rpp() -> RuralPostman<SimpleGraph, i64> {
    // 6 vertices, 8 edges
...
fn test_rural_postman_serialization() {
    ...
    let restored: RuralPostman<SimpleGraph, i64> = serde_json::from_value(json).unwrap();
...
fn test_rural_postman_problem_name() {
    assert_eq!(
        <RuralPostman<SimpleGraph, i64> as Problem>::NAME,
        "RuralPostman"
    );
}
