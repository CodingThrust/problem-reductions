...
fn test_evaluate() {
    ...
    assert_eq!(problem.evaluate(&[1, 0, 1, 0]).unwrap(), Min(Some(2)));
...
fn test_empty_edges() {
    ...
    assert_eq!(problem.evaluate(&[0, 0, 0, 0]).unwrap(), Min(Some(0)));
}
...
fn test_biclique_problem() {
    ...
    assert_eq!(empty_problem.evaluate(&[0, 0, 0, 0]).unwrap(), Min(Some(0)));
}
