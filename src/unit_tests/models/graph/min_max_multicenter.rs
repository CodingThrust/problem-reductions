...
fn example_instance() -> MinMaxMulticenter<SimpleGraph, i64> {
...
fn test_minmaxmulticenter_evaluate_suboptimal() {
    ...
    assert_eq!(problem.evaluate(&[1, 0, 0, 0, 0, 1]).unwrap(), Min(Some(2)));
}
...
fn test_minmaxmulticenter_evaluate_wrong_config_length() {
    let problem = example_instance();
    assert_eq!(problem.evaluate(&[0, 1, 0, 0, 0, 0, 1]).unwrap(), Min(None));
}
...
fn test_minmaxmulticenter_serialization() {
    ...
    let deserialized: MinMaxMulticenter<SimpleGraph, i64> = serde_json::from_str(&json).unwrap();
...
fn test_minmaxmulticenter_disconnected() {
    ...
    assert_eq!(problem2.evaluate(&[1, 0, 1, 0]).unwrap(), Min(Some(1)));
}
...
fn test_minmaxmulticenter_weighted() {
    ...
    // max = 3
    assert_eq!(problem.evaluate(&[0, 1, 0]).unwrap(), Min(Some(3)));

    // Center at 0: d(0)=0, d(1)=1, d(2)=2
    // w(0)*d(0) = 0, w(1)*d(1) = 1, w(2)*d(2) = 4
    // max = 4
    assert_eq!(problem.evaluate(&[1, 0, 0]).unwrap(), Min(Some(4)));
}
...
fn test_minmaxmulticenter_single_vertex() {
    ...
    assert_eq!(problem.evaluate(&[1]).unwrap(), Min(Some(0)));
}
...
fn test_minmaxmulticenter_all_centers() {
    ...
    assert_eq!(problem.evaluate(&[1, 1, 1]).unwrap(), Min(Some(0)));
}
...
fn test_minmaxmulticenter_nonunit_edge_lengths() {
    ...
    assert_eq!(problem.evaluate(&[0, 0, 1]).unwrap(), Min(Some(4)));
}
...
fn create_specs_map_weight_inputs_for_both_variants() {
    let weighted = MinMaxMulticenter::try_from(MinMaxMulticenterI64CreateSpec {
        graph: vec![(0, 1)],
        num_vertices: None,
        weights: None,
    ...
    assert_eq!(MinMaxMulticenterI64CreateSpec::FIELDS[2].name, "weights");
    assert_eq!(
        MinMaxMulticenterI64CreateSpec::FIELDS[3].name,
        "edge_weights"
    );
}
