...
fn test_bipartite_graph_basic() {
    ...
    let g = BipartiteGraph::new(2, 3, edges).unwrap();
...
fn test_bipartite_graph_edges_unified() {
    let g = BipartiteGraph::new(1, 2, vec![(0, 0), (0, 1)]).unwrap();
    let edges = g.edges();
...
fn test_bipartite_graph_has_edge() {
    let g = BipartiteGraph::new(2, 2, vec![(0, 0), (1, 1)]).unwrap();
...
fn test_bipartite_graph_neighbors() {
    let g = BipartiteGraph::new(2, 2, vec![(0, 0), (0, 1), (1, 1)]).unwrap();
...
fn test_bipartite_graph_left_edges() {
    ...
    let g = BipartiteGraph::new(2, 2, edges.clone()).unwrap();
    assert_eq!(g.left_edges(), &edges);
}

#[test]
fn test_bipartite_graph_invalid_left_index() {
    assert!(BipartiteGraph::new(2, 2, vec![(2, 0)]).is_err());
}

#[test]
fn test_bipartite_graph_invalid_right_index() {
    assert!(BipartiteGraph::new(2, 2, vec![(0, 2)]).is_err());
}
...
fn deserialize_checks_partition_endpoints_and_total_size() {
    ...
    assert!(BipartiteGraph::new(usize::MAX, 1, vec![]).is_err());
