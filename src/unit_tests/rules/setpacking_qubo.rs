...
fn test_setpacking_to_qubo_closed_loop() {
    ...
    let sp = SetPacking::<i32>::new(vec![vec![0, 2], vec![1, 2], vec![0, 3]]);
...
fn test_setpacking_to_qubo_disjoint() {
    ...
    let sp = SetPacking::<i32>::new(vec![vec![0, 1], vec![2, 3], vec![4]]);
    let reduction = ReduceTo::<QUBO<f64>>::reduce_to(&sp);
    let qubo = reduction.target_problem();
...
fn test_setpacking_to_qubo_all_overlap() {
    ...
    let sp = SetPacking::<i32>::new(vec![vec![0, 1], vec![0, 2], vec![0, 3]]);
    let reduction = ReduceTo::<QUBO<f64>>::reduce_to(&sp);
    let qubo = reduction.target_problem();
