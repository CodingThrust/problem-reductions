...
fn test_sat_to_minimumdominatingset_closed_loop() {
    ...
    assert_eq!(ds_problem.inner().graph().num_vertices(), 4);
    ...
    assert_eq!(ds_problem.inner().graph().num_edges(), 4);
}
...
fn test_extract_solution_positive_literal() {
    ...
    let sat_sol = reduction
        .recover_result(
            &sat,
            SolveOutcome::optimal(reduction.target_problem(), ds_sol.clone()).unwrap(),
        )
        .unwrap()
        .into_solution()
...
fn test_ds_structure() {
    ...
    assert_eq!(ds_problem.inner().graph().num_vertices(), 11);
}
...
fn test_empty_sat() {
    ...
    assert_eq!(ds_problem.inner().graph().num_vertices(), 0);
    assert_eq!(ds_problem.inner().graph().num_edges(), 0);
...
fn test_multiple_literals_same_variable() {
    ...
    assert_eq!(ds_problem.inner().graph().num_edges(), 5);
}
...
fn test_extract_solution_too_many_selected() {
    ...
    assert!(!reduction.target_problem().evaluate(&ds_sol).unwrap().0);
}

#[test]
fn test_value_mapping_rejects_unselected_variable_gadget() {
    let sat = Satisfiability::new(1, vec![CNFClause::new(vec![1])]);
    let reduction = ReduceTo::<Decision<MinimumDominatingSet<SimpleGraph, i64>>>::reduce_to(&sat)
        .expect("reduction should succeed");
...
#[test]
fn test_value_mapping_rejects_selected_clause_vertex() {
    let sat = Satisfiability::new(1, vec![CNFClause::new(vec![1])]);
    let reduction = ReduceTo::<Decision<MinimumDominatingSet<SimpleGraph, i64>>>::reduce_to(&sat)
        .expect("reduction should succeed");
...
fn test_sat_to_dominatingset_native_certificates() {
    ...
    for (n, clauses) in [
        ...
        for mask in 0..(1usize << target.num_vertices()) {
            ...
            let certificate = value == crate::types::Min(Some(*result.target_problem().bound()));
            assert_eq!(
            ...
            if certificate {
                let x = result
                    ...
                    .into_solution()
                    .expect("qualifying target result must recover a source solution");
                assert_eq!(source.evaluate(&x).unwrap(), crate::types::Or(true));
                accepted = true;
            }
        }
        assert_eq!(
