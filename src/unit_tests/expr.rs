...
#[test]
fn algebraic_analysis_preserves_exact_complexity_facts() {
    let expression = Expr::parse("2^(0.7905 * n)");
    let analysis = AlgebraicAnalysis::new(&[&expression]);
    let ExprNode::Pow(base, exponent) = expression.node() else {
        panic!("expected power expression");
    };

    assert_eq!(
        analysis.facts(base).exact_rational,
        Some(BigRational::from_integer(2.into()))
    );
    assert_eq!(
        analysis.facts(exponent).linear,
        Some(BTreeMap::from([(
            Symbol::new("n").unwrap(),
            BigRational::new(1581.into(), 2000.into()),
        )]))
    );
}

#[test]
fn algebraic_analysis_separates_value_from_domain() {
    let valid = Expr::parse("3^8");
    let invalid = Expr::sqrt(Expr::integer(-1));
    let analysis = AlgebraicAnalysis::new(&[&valid, &invalid]);

    assert!(analysis.facts(&valid).is_constant);
    assert_eq!(analysis.facts(&valid).exact_rational, None);
    assert_eq!(analysis.facts(&valid).constant_domain, Some(true));
    assert_eq!(analysis.facts(&invalid).constant_domain, Some(false));
}
