...
- Aggregate value mappings register with `#[aggregate_reduction]` (or `register_aggregate_reduction!` for generic results), not manual `ReductionEntry` wiring. Manual `ReductionEntry` remains only for Turing edges and metadata-only edges without an executor. An aggregate edge does not imply that a problem may solve successfully without a `Solution`
